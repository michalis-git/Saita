...
void Box2DWorld::timerEvent(QTimerEvent *event)
{
    ...
    if(m_pSaita != 0){
        int saitaRightX = (m_pSaita->x()+m_pSaita->width());
        int saitaBottomY = (m_pSaita->y()+m_pSaita->height());
         // BONUS ITEM COLLISION DETECTION
        QList<Box2DBody*>::const_iterator i;
        for(i = mBodies.constBegin(); i != mBodies.constEnd(); i++){
            Box2DBody *body = *i;

            if(body->bodyType() != Box2DBody::Kinematic)
                continue;

            if(body == m_pSaita)
                continue;

            bool collides = (m_pSaita->x() < body->x()+body->width() && (m_pSaita->y() < body->y() + body->height()))
                    && (saitaRightX > body->x()) && (saitaBottomY > body->y());

            if(collides)
            {
                emit extrasContact(body->property("indexNo").toInt());
...
void Box2DWorld::fetchSaita(){
    QList<Box2DBody*>::const_iterator i;
    for(i = mBodies.constBegin(); i != mBodies.constEnd(); i++){
        Box2DBody *body = *i;

        if(body->property("player").toBool())
        {
            qDebug() << "found";
            m_pSaita = body;
        }
