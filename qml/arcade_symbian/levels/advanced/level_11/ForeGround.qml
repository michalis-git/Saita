...
Flickable{
    ...
    Hand{
        ...
        y: classRoomSaita.y+classRoomSaita.height+21
        width: 60
        height: 53
        z:200
        ...
        property alias handArea: handArea
...
//                backgroundLevel0.x = 0;
            ...
            onPositionChanged: {
                ...
                if(!classroom.running && !paused && mapToItem(foreground,mouse.x,mouse.y).y > 75 && mapToItem(foreground,mouse.x,mouse.y).y < (worldClassroom.height - worldClassroom.floorWallHeight-hand.height/2)){
    ...
    World {
        ...
        property int floorWallHeight: 23
        ...
        onExtrasContact:{
            if(classroom.state === 'lost' || idxNo == 0)
                return;
...
//                // check if the game is running
                if(classroom.running && !classroom.gameFinish){
                    ...
                    if(background1TranslateValue < offsetLimit)
                        classroom.gameFinish = true;
                }else{

                    var offsetX1 = classroom.mapToItem(backgroundLevel1,classroom.width,0).x;
