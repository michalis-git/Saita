...
function applyForce(){
    ...
    if(linearYVelocity > 38) {// check whether the glinder has a velocity greater than 50..
        saita.applyLinearImpulse(Qt.point(0,-force),Qt.point(saita.x + saita.width/2,saita.y+saita.height/2));
    }
    else if(linearYVelocity <= 38 && linearYVelocity > - 38){ // if the linear velocity towards the y axes is less than zero, then apply only one third of the original impulse
...
function applyScissorEffect(specialeffect){
    ...
    levelView.playScissorsSoundEffect();
...
function reset(){
    ...
    saita.x = foreground.hand.width-51
...
//
function activateSaitasArrayElements(){
    console.log('started iterating saita array to activate elements')
    for(var i = 0; i < saitasArray.length; i++){
        var saitaObject = saitasArray[i];

        if(saitaObject != null){
            saitaObject.state = 'active'
        }
    }
}

//
function inActivateSaitasArrayElements(){
    console.log('started iterating saita array to inactivate elements')

    for(var i = 0; i < saitasArray.length; i++){
        var saitaObject = saitasArray[i];

        if(saitaObject != null){
            saitaObject.state = 'inActive'
            console.log('set state to inActive');
        }else
            console.log('saitaObsject is null')
    }
}
        ...
        var object = component.createObject(foreground.worldClassroom,
                                            {
                                                ...
                                                "x": foreground.worldClassroom.width-6.5*foreground.obstacles.firnitureSpacing,
                                                "y": foreground.worldClassroom.height*((4-helperCounter)/7),
                                                "force": ((4-helperCounter)/3)*-35
                                            } );
        object.z = 200
        object.linearVelocity = Qt.point(-150,0)
        object.bottomBorder = foreground.worldClassroom.height* ((4-helperCounter)/6.5)
