...
Item {
    property int spacing: 150

    property variant firstBonusPos: Qt.point(obstacle1.x-1.3*spacing,worldClassroom.ceilingWallHeight)
    property variant secondBonusPos: Qt.point(obstacle7.x + obstacle7.width+spacing/1.4,worldClassroom.height-worldClassroom.ceilingWallHeight-spacing/4.1)
    ...
    function createBasketBall(){
        ...
        if(component.status == Component.Ready){
            var object = component.createObject(worldClassroom,{"x": worldClassroom.width-spacing/2, "y": worldClassroom.height/3, "initialLinearVelocity": Qt.point(-160,112),"bRestitution": 0.98} );
            object.z = 200
        }else
