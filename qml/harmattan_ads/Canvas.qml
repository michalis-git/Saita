...
Item {
    ...
    function showBannerDialog(){
        if(!lostHelperCounter.running && loader.item.score > 50){
            pageMain.bannerDialog.seconds = 6;
        }
    }

    property alias lostHelperTimer: lostHelperCounter

    Timer{
        id: lostHelperCounter
        repeat: true
        running: false
        interval: 30000
    ...
    Connections{
        target: loader.item
        onLost: { lostMenu.score = scVal; lostMenu.bonusItems = bVal }
    ...
    states:[
        State{
            ...
            StateChangeScript{ name: "runningScript"; script: { if(loader.item != null){ loader.item.mArea.enabled = true } lostHelperCounter.start() } }
    ...
    Component.onCompleted: lostHelperCounter.start()
}
